...
import { formatAmountWithSign, formatDayLabel, formatTime } from '../../utils/formatters';
...
export default function TransactionItem({ transaction, onPress }) {
  ...
  const accountSuffix = transaction.accountLast4 ? `•• ${transaction.accountLast4}` : '';
  const subtitle = [bankLabel, accountSuffix, formatDayLabel(transaction.timestamp)]
    .filter(Boolean)
    .join(' · ');
