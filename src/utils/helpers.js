...
export function groupByCategory(transactions) {
  ...
  return Object.entries(map)
    .map(([categoryId, total]) => ({ categoryId, category: getCategoryById(categoryId), total }))
...
export function groupByDay(transactions) {
  ...
  transactions.forEach(t => {
    ...
    if (!map[day]) map[day] = { credit: 0, debit: 0 };
    if (t.type === 'credit') map[day].credit += t.amount;
    else map[day].debit += t.amount;
  });
  return Object.entries(map)
