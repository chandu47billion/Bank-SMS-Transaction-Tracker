...
const buildTxn = ({ day, hour, minute, amount, type, bank, last4, merchant, description, category, balance, refNo }) => ({
  id: nextId(),
  amount,
  type,
  bank,
...
export function generateSeedTransactions() {
  ...
  applyCredit(68000);
  push({ day: 58, hour: 9, minute: 5, amount: 68000, type: 'credit', bank: 'hdfc', last4: '4521', merchant: 'ACME TECHNOLOGIES PVT LTD', description: 'Salary credited for last month', category: 'salary', refNo: 'REF10023481' });

  applyCredit(69500);
  push({ day: 28, hour: 9, minute: 12, amount: 69500, type: 'credit', bank: 'hdfc', last4: '4521', merchant: 'ACME TECHNOLOGIES PVT LTD', description: 'Salary credited for last month', category: 'salary', refNo: 'REF10098213' });
  ...
  foodOrders.forEach(({ day, amt, merchant, bank }) => {
    applyDebit(amt);
    push({ day, hour: 13, minute: 30, amount: amt, type: 'debit', bank, last4: getBankById(bank).id === 'sbi' ? '7845' : '4521', merchant, description: `UPI payment to ${merchant}`, category: 'food', refNo: `UPI${400000000 + day * 37}` });
  });
  ...
  shopping.forEach(({ day, amt, merchant, bank }) => {
    applyDebit(amt);
    push({ day, hour: 19, minute: 45, amount: amt, type: 'debit', bank, last4: '4521', merchant, description: `Payment to ${merchant}`, category: 'shopping', refNo: `TXN${500000000 + day * 41}` });
  });
  ...
  atms.forEach(({ day, amt, bank }) => {
    applyDebit(amt);
    push({ day, hour: 11, minute: 0, amount: amt, type: 'debit', bank, last4: '7845', merchant: 'ATM Withdrawal', description: 'Cash withdrawal at ATM', category: 'atm', refNo: `ATM${600000000 + day * 53}` });
  });

  // --- Bills ---
  const bills = [
    { day: 47, amt: 1850, merchant: 'BESCOM ELECTRICITY', bank: 'hdfc', category: 'bills' },
    { day: 42, amt: 599, merchant: 'AIRTEL POSTPAID', bank: 'sbi', category: 'bills' },
    { day: 30, amt: 799, merchant: 'JIO FIBER', bank: 'icici', category: 'bills' },
    { day: 17, amt: 1920, merchant: 'BESCOM ELECTRICITY', bank: 'hdfc', category: 'bills' },
    { day: 8, amt: 599, merchant: 'AIRTEL POSTPAID', bank: 'sbi', category: 'bills' },
  ];
  bills.forEach(({ day, amt, merchant, bank, category }) => {
    applyDebit(amt);
    push({ day, hour: 8, minute: 15, amount: amt, type: 'debit', bank, last4: '4521', merchant, description: `Bill payment to ${merchant}`, category, refNo: `BIL${700000000 + day * 59}` });
  });
  ...
  fuel.forEach(({ day, amt, bank }) => {
    applyDebit(amt);
    push({ day, hour: 18, minute: 0, amount: amt, type: 'debit', bank, last4: '4521', merchant: 'Indian Oil Petrol Pump', description: 'Fuel purchase', category: 'fuel', refNo: `FUE${800000000 + day * 61}` });
  });
  ...
  medical.forEach(({ day, amt, merchant, bank }) => {
    applyDebit(amt);
    push({ day, hour: 17, minute: 30, amount: amt, type: 'debit', bank, last4: '7845', merchant, description: `Payment to ${merchant}`, category: 'medical', refNo: `MED${900000000 + day * 67}` });
  });
  ...
  entertainment.forEach(({ day, amt, merchant, bank }) => {
    applyDebit(amt);
    push({ day, hour: 20, minute: 0, amount: amt, type: 'debit', bank, last4: '4521', merchant, description: `Payment to ${merchant}`, category: 'entertainment', refNo: `ENT${100000000 + day * 71}` });
  });
  ...
  upiMisc.forEach(({ day, amt, merchant, bank }) => {
    applyDebit(amt);
    push({ day, hour: 12, minute: 10, amount: amt, type: 'debit', bank, last4: '7845', merchant, description: `UPI transfer to ${merchant}`, category: 'upi', refNo: `UPI${200000000 + day * 73}` });
  });
  ...
  travel.forEach(({ day, amt, merchant, bank }) => {
    applyDebit(amt);
    push({ day, hour: 9, minute: 40, amount: amt, type: 'debit', bank, last4: '4521', merchant, description: `Payment to ${merchant}`, category: 'travel', refNo: `TRV${300000000 + day * 79}` });
  });

  // --- Small interest / refund credit ---
  applyCredit(340);
  push({ day: 39, hour: 6, minute: 0, amount: 340, type: 'credit', bank: 'hdfc', last4: '4521', merchant: 'HDFC BANK', description: 'Quarterly interest credited', category: 'others', refNo: 'INT88213' });

  applyCredit(1200);
  push({ day: 13, hour: 15, minute: 20, amount: 1200, type: 'credit', bank: 'icici', last4: '7845', merchant: 'AMAZON REFUND', description: 'Refund for returned item', category: 'others', refNo: 'RFD44521' });
